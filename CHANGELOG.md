...
- **deep-review** 1.1.1 -> 1.2.0: a Red Flag covering the hardcoded-**false** direction. The existing vacuous-test warning catches a guard stuck ON — the negative fixture fails — but says nothing about one stuck OFF, where an "expect nothing" assertion passes precisely *because* the detector is dead. The new entry requires both directions, prefers proving it by mutation over inspection, and adds a third axis the issue's own proposal was missing: **fixture reachability**. PR #91 supplied the counterexample — its `--add ,` assertion had a positive control *and* a fail-first negative and was still blind, because `MONOREPO_ADDCOMMA_FIXTURE` was created bare, so it exercised only the half that already worked (`--add ,` refused against an empty monorepo, exited 0 with a "Sync complete" banner against a populated one). No stricter assertion against that fixture could have caught it. Also reconciles the version sites **forward**: the live `plugin-manifest.json` was still 1.0.0 while the derived `plugin.json`/`marketplace.json` read 1.1.1 and the README row read 1.0.0, so a sync run would have regressed the published version 1.1.1 -> 1.0.0; and `~/.claude/skills/deep-review/CHANGELOG.md` is created for the first time, since without it the publishing tooling fabricates a stub that would destroy the entry on the next sync (#82). `ship`'s duplicated Phase 4 wording is deliberately left alone — it is unversioned, out-of-tree, and was the orchestrator of the session that made this change. (#69)
