...
- `scripts/test-sync-hygiene.sh` — regression harness for the `skill-publishing` sync-hygiene fixes above. Builds a throwaway `SKILLS_HOME` and four throwaway monorepos and runs the real `sync-monorepo.sh` against them from a throwaway cwd — a plain sync, an `--add` sync (the second, otherwise-uncovered discovery site), a sync of a monorepo holding no skills at all, and a sync of one whose only skill directory is named `-n`. Asserts the non-skill filtering at both sites, the absent `build/` leftover in the caller's cwd, the auto-build temp stage's cleanup, both `.gitignore` sites, CHANGELOG top-entry preservation, the empty-list count, and authoring-copy parity — each behind a positive control that the corresponding stage actually ran and wrote to disk. `gh` is shimmed off `PATH` so the run reaches no network. Every assertion was proven red against a build with the matching fix reverted. (#74)
