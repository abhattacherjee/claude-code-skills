...
- **skill-publishing** 4.2.0 -> 4.2.1: `sync-monorepo.sh` treated every top-level monorepo directory as a skill, so `docs/` and `build/` entered the sync loop and emitted a spurious `ERROR: no SKILL.md` line on every run; the plugin auto-build stage assembled into `./build/<name>` in the caller's cwd, leaving an untracked `build/` tree that the script's own `git add -A` next-step banner would commit; and the `.gitignore` template written into a freshly `--init`-ed monorepo omitted `build/`. Discovery now filters non-skill directories (announcing each on stderr), auto-builds go to a `mktemp -d` stage cleaned up by an EXIT trap, and both `.gitignore` sites list `build/`. (#74)
- The monorepo's own `.gitignore` now ignores `build/`, so a `prepare-plugin.sh` run from the repo root no longer leaves a committable untracked build tree. (#74)
- **skill-publishing** 4.2.1: `sync-monorepo.sh` printed `echo: write error: Broken pipe` to stderr on every run against a CHANGELOG larger than the 64 KiB pipe buffer — a latent `| head -1` that only became visible once the auto-build EXIT trap kept the subshell alive past SIGPIPE. Replaced with a parameter expansion; generated output is byte-identical. An empty skill list also reported `Skills to sync (1)` and a bare `  - ` bullet. (#74)
...
- `scripts/test-sync-hygiene.sh` — regression harness for the `skill-publishing` sync-hygiene fixes above. Builds a throwaway `SKILLS_HOME` and monorepo, runs the real `sync-monorepo.sh` from a throwaway cwd, and asserts the non-skill filtering, the absent `build/` leftover, both `.gitignore` sites, and authoring-copy parity — with a positive control that a real skill is still synced. (#74)
