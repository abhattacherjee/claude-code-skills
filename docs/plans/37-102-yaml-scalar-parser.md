...
is why the leak is not visible in git. A full `sync-monorepo.sh` run auto-builds
the plugin and `rsync -a --delete`s over `plugins/deep-review/`, so this is a
live regression waiting on the next routine sync, not a latent one.
...
3. If the remaining value is a **block-scalar indicator**
   (`/^[|>][0-9]*[+-]?[ \t]*(#.*)?$/`), consume the following indented lines,
   stopping at the first non-indented line (per YAML) and skipping blanks.
   Join with `" "` for `>` (folded) and `"\n"` for `|` (literal).
4. Otherwise, strip quotes **only when the value genuinely opens and closes with
   ...
   - `"…"` → `\n`/`\t`/`\r` become the control characters; every other
     `\<c>` becomes a literal `<c>`, which covers `\"`, `\\` and `\/`.
   - `'…'` → `''` collapses to `'`.
   - anything else → returned verbatim, quotes and all.
5. Emit with `printf "%s\n", …`, never `print`-into-a-format, so a `%` in a
   description cannot be read as a conversion.
...
- A value that both starts and ends with `"` but is not actually a quoted
  scalar (`"a" or "b"`) is treated as quoted. That document is invalid YAML
  anyway; the rule is "same quote at both ends", per the design above.
...
- [x] **B** — rewrite `extract_field` in `_lib.sh`; fix `short_desc` to use
  ...
  They are byte-identical today and `test-sync-hygiene.sh:4284` asserts it, so a
  one-sided edit turns that assertion red.
- [x] **C** — regression fixtures in `scripts/test-sync-hygiene.sh` driving the
  real `prepare-plugin.sh` through `run_prepare` (line 1550).
- [x] **D** — prove fail-first: run the suite with the OLD parser restored and
  record which new assertions go red.
