...
- `codex-review.sh --mode find|judge|counter` runs Codex in a locked-down `codex exec`: an environment of only `PATH`, `HOME`, your own `CODEX_HOME` and, when set, `OPENAI_API_KEY` and `CODEX_API_KEY`, `HTTP_PROXY`, `http_proxy`, `HTTPS_PROXY`, `https_proxy`, `NO_PROXY`, `no_proxy` and `TMPDIR`; user config, rules, the reviewed repo's `AGENTS.md`, apps, plugins, hooks and memories off; a read-only sandbox; the diff on a closed stdin pipe; and a timeout that kills the whole process group. Its output is checked against a schema, capped and redacted. Codex finding ids are `X-001…`.
