...
- an environment holding `PATH`, `HOME` and your own `CODEX_HOME` (as set, else Codex's default `~/.codex`), plus each of these that your shell sets to a non-empty value: `CODEX_API_KEY` (an API-key login), `HTTP_PROXY`, `http_proxy`, `HTTPS_PROXY`, `https_proxy`, `NO_PROXY` and `no_proxy` (proxy settings), and `TMPDIR` (a scratch dir). Nothing else from your shell reaches Codex. `OPENAI_API_KEY` is deliberately not passed: an unrelated key set for some other tool would override the login already in `CODEX_HOME` and make `codex exec` fail. `CODEX_API_KEY` does reach it when set, so unset it for the review if you log in with ChatGPT and do not want it passed;
...
Each receives: absolute path to `DIFF_FILE`, absolute path to `FILES_FILE`, and read access to the repo. Also tell each: run long harnesses (mutation runs, fuzzers, full suites) in the foreground, with the Bash timeout near its maximum. If a run will take more than about 20 minutes, split it into chunks and send partial results after each chunk. Never go idle "waiting for your background run": an idle teammate is not woken when its own job ends.
...
- Repo read access
- The same rule as Step 2: run long harnesses in the foreground with the Bash timeout near its
  maximum, chunk anything over ~20 minutes with partial results after each chunk, and never go
