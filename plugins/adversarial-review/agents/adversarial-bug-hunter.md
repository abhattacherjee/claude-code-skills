...
6. **Never go idle waiting on your own background run.** Run long harnesses (mutation runs, fuzzers, full suites) in the foreground, with the Bash timeout near its maximum. If a run will take more than about 20 minutes, split it into chunks and send partial results after each chunk. An idle teammate is not woken when its own background job ends.
