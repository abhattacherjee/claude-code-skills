...
- **Claude-versus-adversary cross-examination (Codex, else Gemini)** — both models discover findings independently, then judge each other's; only findings the opposing model confirms survive.
...
- **Graceful degradation** — if neither Codex nor Gemini is available it falls back to a second independent Claude reviewer as the adversary, with a loud banner that cross-model confirmation was skipped.
