...
- Reviewer dispatch, re-review, the Fix/Step 2.5 implementer dispatch, and the Phase 2 R1/R2 briefs now tell reviewers and implementers to run long harnesses (mutation runs, fuzzers, full suites) in the foreground, keeping each Bash call under the 10-minute cap (chain calls, or split into chunks, rather than backgrounding it), and send partial results at least every ~20 minutes of a long run, and never go idle waiting on their own background run. The orchestrator checks a reviewer's or implementer's background job (`ps -axo pid,etime,command`, or its scratch output) within about 10 minutes before reporting it is waiting on one.
